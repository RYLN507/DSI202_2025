...
class PostForm(forms.ModelForm):
    # เอา category เข้าไปเป็น ModelChoiceField
    category = forms.ModelChoiceField(
        queryset=PostCategory.objects.all(),
        required=False,
        widget=forms.Select(attrs={
            'class': 'px-4 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#3F8240] bg-white'
        })
    )
