...
from .models import Category, Restaurant, Menu  # import Menu ด้วย

def shop_list(request, category_id=None, category_name=None):
    # เดิมหา category ว่ายังไงก็ได้ตามก่อนหน้า
    ...
    if category:
        ...
        # **หาเมนูทั้งหมดในหมวดนี้** ผ่าน relation restaurant→menu
...
def product_filter(request):
    ...
    price_level         = request.GET.get('price', '')
    allergy_note        = request.GET.get('allergy_note', '').strip()

    no_milk    = request.GET.get('no_milk')    == '1'
    no_egg     = request.GET.get('no_egg')     == '1'
    no_peanut  = request.GET.get('no_peanut')  == '1'
    no_gluten  = request.GET.get('no_gluten')  == '1'
    no_seafood = request.GET.get('no_seafood') == '1'
    no_fish    = request.GET.get('no_fish')    == '1'

    # 2) Build your base queryset
    qs = Menu.objects.select_related('restaurant','restaurant__category').all()

    # 3) Apply filters
    if selected_categories:
        qs = qs.filter(restaurant__category__name__in=selected_categories)

    if price_level == '0':
        qs = qs.filter(discount_price__lte=100)
    elif price_level == '1':
        qs = qs.filter(discount_price__gt=100, discount_price__lte=500)
    elif price_level == '2':
        qs = qs.filter(discount_price__gt=500)

    if no_milk:    qs = qs.exclude(description__icontains='นม')
    if no_egg:     qs = qs.exclude(description__icontains='ไข่')
    if no_peanut:  qs = qs.exclude(description__icontains='ถั่ว')
    if no_gluten:  qs = qs.exclude(description__icontains='กลูเตน')
    if no_seafood: qs = qs.exclude(description__icontains='ทะเล')
    if no_fish:    qs = qs.exclude(description__icontains='ปลา')

    if allergy_note:
        qs = qs.exclude(description__icontains=allergy_note)

    # 4) Render the *results* template
...
def board_list(request):
    ...
    if cat:
        # only valid categories in the dropdown will filter
...
def delete_post(request, pk):
    ...
    return render(request, 'community/post_confirm_delete.html', {
        'post': post
    })
...
class PostForm(forms.ModelForm):
    ...
    category = forms.ModelChoiceField(
        ...
        widget=forms.Select(attrs={
            'class': 'px-3 py-2 border border-gray-300 rounded-full bg-white focus:outline-none focus:ring-2 focus:ring-[#3F8240]'
        })
    )
